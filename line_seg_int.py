def intersect_coords(segment_1: tuple, segment_2: tuple) -> (float, float):
    ...
    min_x = max(min(x_1, x_2), min(x_3, x_4))
    max_x = min(max(x_1, x_2), max(x_3, x_4))
    min_y = max(min(y_1, y_2), min(y_3, y_4))
    max_y = min(max(y_1, y_2), max(y_3, y_4))
...
if __name__ == "__main__":
    # demo
    segment_1 = ((2.0, 3.0), (5.0, 2.0))
    segment_2 = ((5.0, 1.0), (1.0, 4.0))
    print(f"Segments {segment_1} and {segment_2} intersect at {intersect_coords(segment_1, segment_2)}")
